...
func GetSalesAttendanceFromDailyTask(day string) []SalesAttendance {
	...
	switch day {
	case "today":
		query = "select employee_id, InTime, OutTime from ( select daystarttype DayStartType,date_format(InTime,'%Y-%m-%d') Date,UserErpId,PunchDate, sales_mst.new_e_code employee_id, min(case when ActivityType='Day End (Normal)' then OutTime else case when InTime > PunchDate then InTime else case when OutTime > PunchDate then OutTime else NULL end end end) InTime, max(case when ActivityType='Day Start' then InTime else case when OutTime > PunchDate then OutTime else case when InTime > PunchDate then InTime else NULL end end end) OutTime from microtek.dailytasks as tasks, microtek.sales_employee_mapping as sales_mst where tasks.UserErpId = sales_mst.sales_id and PunchDate >= date_format(current_date(), '%Y-%m-%d') group by daystarttype,PunchDate,UserErpId order by daystarttype,usererpid,PunchDate) tt;"
	case "yesterday":
		query = "select employee_id, InTime, OutTime from ( select daystarttype DayStartType,date_format(InTime,'%Y-%m-%d') Date,UserErpId,PunchDate, sales_mst.new_e_code employee_id, min(case when ActivityType='Day End (Normal)' then OutTime else case when InTime > PunchDate then InTime else case when OutTime > PunchDate then OutTime else NULL end end end) InTime, max(case when ActivityType='Day Start' then InTime else case when OutTime > PunchDate then OutTime else case when InTime > PunchDate then InTime else NULL end end end) OutTime from microtek.dailytasks as tasks, microtek.sales_employee_mapping as sales_mst where tasks.UserErpId = sales_mst.sales_id and PunchDate >= date_format(current_date() - INTERVAL 1 DAY, '%Y-%m-%d') group by daystarttype,PunchDate,UserErpId order by daystarttype,usererpid,PunchDate) tt;"
	default:
...
func GetSalesAttendanceFromDailyTaskUnmatched(day string) []SalesAttendance {
	...
	switch day {
	case "today":
		query = "select UserErpId as employee_id, min(case when ActivityType='Day End (Normal)' then OutTime else  case when InTime > PunchDate then InTime else case when OutTime > PunchDate then OutTime else NULL end end end) InTime, max(case when ActivityType='Day Start' then InTime else case when OutTime > PunchDate then OutTime else case when InTime > PunchDate then InTime else NULL end end end) OutTime from microtek.dailytasks as tasks where PunchDate >= date_format(current_date(), '%Y-%m-%d') and UserErpId Not In (select distinct(sales_id) from microtek.sales_employee_mapping) group by daystarttype,PunchDate,UserErpId order by daystarttype,usererpid,PunchDate;"
	case "yesterday":
		query = "select UserErpId as employee_id, min(case when ActivityType='Day End (Normal)' then OutTime else  case when InTime > PunchDate then InTime else case when OutTime > PunchDate then OutTime else NULL end end end) InTime, max(case when ActivityType='Day Start' then InTime else case when OutTime > PunchDate then OutTime else case when InTime > PunchDate then InTime else NULL end end end) OutTime from microtek.dailytasks as tasks where PunchDate >= date_format(current_date() - INTERVAL 1 DAY, '%Y-%m-%d') and UserErpId Not In (select distinct(sales_id) from microtek.sales_employee_mapping) group by daystarttype,PunchDate,UserErpId order by daystarttype,usererpid,PunchDate;"
	default:
		log.Fatal("Invalid day argument. Must be 'today' or 'yesterday'.")
	}
